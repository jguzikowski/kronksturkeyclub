...
class DraftManager:
    ...
    def start_draft(self, draft_order, channel_id):
...
async def start_draft(ctx, *user_mentions):
    """
    Start a new draft with the specified user order
    Usage: !startdraft @User1 @User2 @User3 ...
    """
    if not user_mentions:
        await ctx.send("❌ Please mention users in draft order: `!startdraft @User1 @User2 @User3`")
        return
    ...
    draft_manager.start_draft(draft_order, ctx.channel.id)
    ...
    embed.add_field(name="Total Picks", value=str(len(draft_order)), inline=True)
    ...
    await ctx.send(f"🔔 <@{current_user_id}> - You're on the clock! Make your pick with `!pick PlayerName, TeamName`")
    if next_user_id:
...
async def make_pick(ctx, *, player_info):
    ...
    # Record the pick
    draft_manager.add_pick(player_name, player_team)
...
async def draft_status(ctx):
    ...
    embed.add_field(name="On the Clock", value=f"<@{current_user_id}>", inline=True)
    if next_user_id:
        embed.add_field(name="On Deck", value=f"<@{next_user_id}>", inline=True)
...
async def help_command(ctx):
    ...
    commands_list = [
        ("!startdraft @User1 @User2 ...", "Start a new draft with user order"),
